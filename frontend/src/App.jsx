...
import { Sparkles, Plus, Search, Upload, Network, Brain } from 'lucide-react';
...
function App() {
    ...
    return (
        <div className="flex h-screen w-full bg-surface text-on-surface overflow-hidden font-sans">
            ...
            <div className="w-20 lg:w-64 border-r border-surface-variant flex flex-col bg-[#1A1A1A] p-4 lg:p-6 shrink-0 transition-all text-on-surface">
                ...
                <nav className="flex-1 space-y-2 overflow-y-auto invisible-scrollbar">
                    ...
                    <div className="pt-4 pb-4 space-y-4 hidden lg:block border-t border-surface-variant/30">
                ...
                <button
                    onClick={() => { setEditingSource(null); setIsCaptureOpen(true); }}
                ...
                >
                    <Plus size={24} />
                    <span className="hidden lg:inline text-sm text-surface">Capture</span>
                </button>
            </div>

            <div className="flex-1 flex flex-col min-w-0">

                <header className="h-20 border-b border-surface-variant flex items-center px-8 bg-[#1F1F1F]/80 backdrop-blur-md z-30 shrink-0">
                    <div className="flex-1 max-w-2xl relative">
                        ...
                        <input
                            ...
                            className="w-full bg-surface-variant/40 border border-transparent focus:border-blue-500/30 rounded-full py-2.5 lg:py-3 pl-10 lg:pl-12 pr-6 text-xs lg:text-sm focus:outline-none transition-all placeholder:text-muted-foreground/50"
                        />
                    </div>
                    <div className="ml-auto flex items-center gap-4">
                        <div className="w-10 h-10 rounded-full border border-surface-variant overflow-hidden cursor-pointer hover:border-primary-container transition-colors ring-4 ring-transparent hover:ring-blue-500/5">
                    ...
                    {activeSource ? (
                        <div className="flex-1 flex overflow-hidden">
                            ...
                            <div className="hidden xl:block">
                                <AIStudio activeSource={activeSource} />
                            </div>
                        </div>
                    ) : (
                        <DashboardGrid
